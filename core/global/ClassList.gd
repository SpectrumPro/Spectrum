...
## Contains the class names of all components that are data containers, and do not effect anything them self. Like Cues, FixtureGroupItem, ect
var data_container_types: Array = [
	"Cue",
	"FixtureGroupItem"
]
...
func _ready() -> void:
	...
	Client.connected_to_server.connect(func ():
		Client.send_command("classlist", "get_global_class_list_keys", [], func(classes: Dictionary): 
