...
@onready var h_seprator: Control = $VBoxContainer/HBoxContainer2/PanelContainer2/ScrollContainer/VBoxContainer/Seprator
...
func clear_rows() -> void:
	for row_item: RowIndex in row_item_container.get_children():
...
func clear_cells() -> void:
	...
	cell_item_container.add_child(h_seprator.duplicate())
...
func create_column(column_name: String) -> ColumnIndex:
	...
	(func ():
		cell_item_container.custom_minimum_size = Vector2(new_column.position.x + new_column.size.x, 0)
	).call_deferred()
